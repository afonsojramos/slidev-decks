...
export async function dev(query?: string, options: { open?: boolean; port?: string; latest?: boolean } = {}) {
  ...
  if (options.latest) {
    const deck = decks[0]; // already sorted by date descending
    console.log(`  ${pc.green("✓")} Latest: ${pc.bold(deck.name)}\n`);
    outro(`Starting ${pc.bold(deck.name)}`);
    const args = [...(options.open ? ["--open"] : []), ...(options.port ? ["--port", options.port] : [])];
